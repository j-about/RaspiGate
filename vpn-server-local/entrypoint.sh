...
set_var EASYRSA_DIGEST \"sha512\"" > vars
    ...
    ./easyrsa gen-req $VPN_SERVER_LOCAL_EASYRSA_SERVER_NAME nopass
    yes yes | ./easyrsa sign-req server $VPN_SERVER_LOCAL_EASYRSA_SERVER_NAME 
    ./easyrsa gen-dh
    ...
    echo "local $VPN_SERVER_LOCAL_STATIC_IP
port $VPN_SERVER_LOCAL_PORT
proto $VPN_SERVER_LOCAL_PROTOCOL
dev tun
ca /etc/openvpn/pki/ca.crt
cert /etc/openvpn/pki/issued/$VPN_SERVER_LOCAL_EASYRSA_SERVER_NAME.crt
key /etc/openvpn/pki/private/$VPN_SERVER_LOCAL_EASYRSA_SERVER_NAME.key
dh /etc/openvpn/pki/dh.pem
tls-auth /etc/openvpn/pki/ta.key 0
server $VPN_SERVER_LOCAL_NETWORK $VPN_SERVER_LOCAL_NETMASK
...
dev tun
proto $VPN_SERVER_LOCAL_PROTOCOL
remote $VPN_SERVER_LOCAL_HOST $VPN_SERVER_LOCAL_PORT
