...
set_var EASYRSA_DIGEST \"sha512\"" > vars
    ...
    ./easyrsa gen-req $VPN_SERVER_VISP_EASYRSA_SERVER_NAME nopass
    yes yes | ./easyrsa sign-req server $VPN_SERVER_VISP_EASYRSA_SERVER_NAME 
    ./easyrsa gen-dh
    ...
    echo "local $VPN_SERVER_VISP_STATIC_IP
port $VPN_SERVER_VISP_PORT
proto $VPN_SERVER_VISP_PROTOCOL
dev tun
ca /etc/openvpn/pki/ca.crt
cert /etc/openvpn/pki/issued/$VPN_SERVER_VISP_EASYRSA_SERVER_NAME.crt
key /etc/openvpn/pki/private/$VPN_SERVER_VISP_EASYRSA_SERVER_NAME.key
dh /etc/openvpn/pki/dh.pem
tls-auth /etc/openvpn/pki/ta.key 0
server $VPN_SERVER_VISP_NETWORK $VPN_SERVER_VISP_NETMASK
push \"redirect-gateway def1\"
push \"route $NETWORK $NETMASK\"
push \"dhcp-option DNS $ADGUARD_STATIC_IP\"
keepalive 10 120
cipher AES-256-GCM
auth SHA512
user nobody
...
duplicate-cn" > server.conf

    echo "client
dev tun
proto $VPN_SERVER_VISP_PROTOCOL
remote $VPN_SERVER_VISP_HOST $VPN_SERVER_VISP_PORT
